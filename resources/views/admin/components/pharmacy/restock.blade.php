...
<div id="modalDetail" class="modal-overlay">
    <div class="modal-container" style="max-width:500px;">
        ...
        <div class="modal-footer" style="padding: 12px 24px; margin-top: 0;">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    let allRows     = [];
    let searchQuery = '';
    let kodeCounter = 1; // untuk generate kode RST-xxx sementara
    ...
    function renderTabel() {
        ...
        tbody.innerHTML = data.map((r, i) => {
            ...
            const harga    = r.purchase_price ? 'Rp ' + Number(r.purchase_price).toLocaleString('id-ID') : '-';
            ...
            return `<tr>
                ...
                <td>
                    <button class="apt-btn-sm"
                        onclick="lihatDetail('${r.id}', '${kode}', '${noFaktur}', '${r.restock_type}', '${tglKirim}', '${tglBuat}', '${supplier}', '${nama}', ${jumlah ?? 0}, '${checker}', '${harga}', '${tempo}', '${r.batch_number ?? '-'}')">
                        Detail
                    </button>
                </td>
            </tr>`;
    ...
    // ── MODAL DETAIL ─────────────────────────────────────────
    window.lihatDetail = function (id, kode, noFaktur, jenis, tglKirim, tglBuat, supplier, nama, jumlah, checker, harga, tempo, batch) {
        const rows = [
            ['Kode', kode],
            ['No Faktur', noFaktur],
    ...
    form.onsubmit = async (e) => {
        ...
        // Simpan semua info ke notes agar bisa ditampilkan di tabel
        const notes = [
