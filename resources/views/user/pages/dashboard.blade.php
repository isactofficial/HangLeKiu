            ...
            <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
                <div class="xl:col-span-2 space-y-6">
                    ...
                    <div class="bg-white rounded-2xl shadow-md border border-slate-100 overflow-hidden">
                        ...
                        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            ...
                            <div class="rounded-lg bg-slate-50 p-4">
                                <p class="text-slate-500">No. Rekam Medis</p>
                                <p class="font-semibold text-slate-800">{{ $patient->medical_record_number ?? '-' }}</p>
                            </div>
                            <div class="rounded-lg bg-slate-50 p-4">
                                <p class="text-slate-500">Total Kunjungan</p>
                                <p class="font-semibold text-slate-800">{{ $totalVisits }} kunjungan</p>
                            </div>
                        </div>
                    </div>
                            ...
                            @if($upcomingAppointment)
                                ...
                                <div class="rounded-xl border border-slate-200 p-4">
                                    ...
                                    <p class="text-sm font-semibold text-slate-800">
                                        {{ \Carbon\Carbon::parse($upcomingAppointment->appointment_datetime)->format('d M Y, H:i') }}
                                    </p>
                                </div>
                                <div class="rounded-xl border border-slate-200 p-4">
                                    <p class="text-xs text-slate-500 uppercase tracking-wide">Dokter</p>
                                    <p class="text-sm font-semibold text-slate-800">{{ $upcomingAppointment->doctor->name ?? '-' }}</p>
                                </div>
                                <div class="rounded-xl border border-slate-200 p-4 md:col-span-2">
                                    ...
                                    @php
                                        ...
                                        $curr = $statusFormat[strtolower($upcomingAppointment->status)]
                                            ?? ['bg-slate-100', 'text-slate-700', ucfirst($upcomingAppointment->status)];
                                    @endphp
                                    <p class="inline-flex items-center mt-2 rounded-full px-3 py-1 {{ $curr[0] }} {{ $curr[1] }} text-sm font-semibold">
                                        {{ $curr[2] }}
                                    </p>
                                </div>
                            @else
                    ...
                    {{-- ── RIWAYAT MEDIS & ODONTOGRAM ── --}}
                    <div class="bg-white rounded-2xl shadow-md border border-slate-100 overflow-hidden">
                        <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
                            <h2 class="text-xl font-semibold text-slate-800">Riwayat Medis & Odontogram</h2>
                            <a href="{{ route('user.medical-history') }}" class="text-sm text-cyan-600 hover:underline">Lihat Semua</a>
                        </div>
                        <div class="p-6 grid grid-cols-1 lg:grid-cols-2 gap-6">

                            {{-- Riwayat Kunjungan --}}
                            <div>
                                <h3 class="font-semibold text-slate-800 mb-3">5 Kunjungan Terakhir</h3>
                                <div class="space-y-3">
                                    @forelse($recentAppointments as $appt)
                                        <div class="border border-slate-200 rounded-xl p-4">
                                            <p class="text-sm font-medium text-slate-800">
                                                {{ \Carbon\Carbon::parse($appt->appointment_datetime)->format('d M Y') }}
                                                — {{ $appt->doctor->name ?? '-' }}
                                            </p>
                                            @php
                                                $notes = $appt->medicalProcedures
                                                    ->flatMap(fn($mp) => $mp->doctorNotes)
                                                    ->first();
                                            @endphp
                                            @if($notes)
                                                <p class="text-sm text-slate-600 mt-1">{{ Str::limit($notes->notes, 80) }}</p>
                                            @else
                                                <p class="text-xs text-slate-400 mt-1 italic">Belum ada catatan dokter.</p>
                                            @endif
                                        </div>
                                    @empty
                                        <p class="text-sm text-slate-500 italic">Belum ada riwayat kunjungan.</p>
                                    @endforelse
                                </div>
                            </div>

                            {{-- Odontogram — semua gigi dari record terakhir --}}
                            <div>
                                <h3 class="font-semibold text-slate-800 mb-3">Odontogram Terakhir</h3>
                                @if($latestOdontogram)
                                    <div class="rounded-xl border border-dashed border-cyan-300 bg-cyan-50 p-5 flex flex-col gap-3">
                                        <p class="text-sm text-slate-600">
                                            <span class="font-medium">Tanggal Periksa:</span>
                                            {{ \Carbon\Carbon::parse($latestOdontogram->examined_at)->format('d M Y') }}
                                        </p>
                                        @if($latestOdontogram->teeth->isNotEmpty())
                                            <div class="flex flex-wrap gap-1.5">
                                                @foreach($latestOdontogram->teeth as $tooth)
                                                    <span class="bg-cyan-100 text-cyan-800 text-xs px-2.5 py-1 rounded-full font-medium">
                                                        Gigi {{ $tooth->tooth_number }}: {{ $tooth->condition ?? '-' }}
                                                    </span>
                                                @endforeach
                                            </div>
                                        @else
                                            <p class="text-sm text-slate-400 italic">Tidak ada data gigi tercatat.</p>
                                        @endif
                                        <a href="{{ route('user.odontogram-history.detail', $latestOdontogram->id) }}"
                                           class="self-start px-4 py-2 rounded-lg bg-cyan-600 hover:bg-cyan-700 text-white font-medium transition text-sm">
                                            Lihat Detail
                                        </a>
                                    </div>
                                @else
                                    <div class="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-5 flex items-center justify-center text-center">
                                        <p class="text-sm text-slate-500 italic">Belum ada data odontogram.</p>
                                    </div>
                                @endif
                            </div>
                ...
                <aside class="space-y-6">
                    <div class="bg-white rounded-2xl shadow-md border border-slate-100 overflow-hidden">
                        ...
                        <div class="p-6">
                            <p class="text-sm text-slate-600">Daftarkan kunjungan baru untuk pemeriksaan atau tindakan gigi.</p>
